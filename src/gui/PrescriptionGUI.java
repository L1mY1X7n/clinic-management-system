...
public class PrescriptionGUI extends JFrame {
    ...
    private void initComponents() {
        ...
        jTable1 = new JTable();
        jLabel1 = new JLabel("Record ID");
        jLabel2 = new JLabel("Patient ID");
        jLabel3 = new JLabel("Medicine");
        jLabel4 = new JLabel("Dosage");
        ...
        add(jLabel1);
        txtRecordId.setBounds(150, 270, 240, 25);
        add(txtRecordId);

        jLabel2.setBounds(30, 305, 110, 25);
        add(jLabel2);
        txtPatientId.setBounds(150, 305, 240, 25);
        add(txtPatientId);

        jLabel3.setBounds(30, 340, 110, 25);
        add(jLabel3);
        txtMedicineName.setBounds(150, 340, 240, 25);
        add(txtMedicineName);

        jLabel4.setBounds(30, 375, 110, 25);
        add(jLabel4);
        txtDosage.setBounds(150, 375, 240, 25);
        add(txtDosage);

        jLabel5.setBounds(30, 410, 110, 25);
        add(jLabel5);
        txtInstructions.setBounds(150, 410, 240, 25);
        add(txtInstructions);

        btnSave.setBounds(460, 320, 180, 30);
        btnSave.addActionListener(evt -> savePrescription());
        add(btnSave);
    }

    private void loadPrescriptionTable() {
        String[] columnNames = {"Prescription ID", "Record ID", "Patient ID",
        ...
        for (Prescription prescription : prescriptions) {
            model.addRow(new Object[]{prescription.getPrescriptionId(),
                prescription.getRecordId(), prescription.getPatientId(),
                prescription.getDoctorId(), prescription.getDate(),
    ...
    private void savePrescription() {
        if (txtRecordId.getText().trim().isEmpty()
            ...
            JOptionPane.showMessageDialog(this,
                    "Record ID, patient ID, medicine, and dosage are required.");
            return;
        }
        Prescription prescription = new Prescription(
                doctorService.generatePrescriptionId(),
                txtRecordId.getText().trim(), txtPatientId.getText().trim(),
